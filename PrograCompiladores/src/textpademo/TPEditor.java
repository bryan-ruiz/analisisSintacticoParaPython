...
public class TPEditor {
    ...
    public TPEditor() {
        ...
        jFrame = new JFrame("Progra 1 compiladores");
        ...
        c.add(new JScrollPane(jTextArea));    //añade el area de edición en el CENTRO
        c.add(new JScrollPane(jTextAreaError), BorderLayout.PAGE_END);    //añade el area de edición en el CENTRO
        c.add(statusBar, BorderLayout.LINE_END);                      //añade la barra de estado, orientación SUR
    ...
    private void buildTextAreaError() {
        jTextAreaError = new JTextArea();    //construye un JTextArea
        ...
        //asigna el manejador de eventos para el cursor
        jTextAreaError.addCaretListener(eventHandler);
        //asigna el manejador de eventos para el ratón
        jTextAreaError.addMouseListener(eventHandler);
        //asigna el manejador de eventos para registrar los cambios sobre el documento
        jTextAreaError.getDocument().addUndoableEditListener(eventHandler);

        //remueve las posibles combinaciones de teclas asociadas por defecto con el JTextArea
        jTextAreaError.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_X, ActionEvent.CTRL_MASK), "none");    //remueve CTRL + X ("Cortar")
        jTextAreaError.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_C, ActionEvent.CTRL_MASK), "none");    //remueve CTRL + C ("Copiar")
        jTextAreaError.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_V, ActionEvent.CTRL_MASK), "none");    //remueve CTRL + V ("Pegar")
    }
